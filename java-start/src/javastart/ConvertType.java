package javastart;

import java.util.Scanner; //Scanner 클래스 사용 
// ctrl+shift+O(알파벳)

public class ConvertType {

	public static void main(String[] args) {
		Scanner sc = new Scanner(System.in); //키보드로 입력위해 //nextLine()는 문자열 1줄
		//import java.util.Scanner
		
		//문자열 입력 
		System.out.println("숫자를 문자로 입력하세요");
	    String d = sc.nextLine(); //
	    //문자열 -> 숫자 변환
	    	double num = Double.parseDouble(d); //a(문자)->num(정수)
	    	//Integer(정수) : wrapper class(래퍼 클래스)
	    	//Wrapper class:기본형을 객체로 감싸는 클래스
	    	//Integer.parseInt(a) : 문자 -> 정수 변환
	    	
	    //숫자 -> 문자열
	    	String str1 = Double.toString(num); //toString (문자로 변환)
	    					//toString 앞에 타입 지정
	    	String str2 = String.valueOf(num); //valueOf(문자로 변환)
	    					   //valueOf :타입 관계없음
	    	
	    //출력
	    	System.out.println("입력 문자열: "+d);
	    	System.out.println("숫자로 변환: "+num);
	    	System.out.println("다시 문자열로 변환(toString): "+str1);
	    	System.out.println("다시 문자열로 변환(valueOf): "+str2);
	    	
	    	sc.close();
		
	}

}
